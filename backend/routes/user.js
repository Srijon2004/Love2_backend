...
router.post("/girlfriend", auth, upload.single("photo"), async (req, res) => {
  try {
    ...
    const newGirlfriend = { name, photo: photoUrl, details: details || "" };
    user.girlfriends.push(newGirlfriend);
    await user.save();
...
router.put("/girlfriend/:girlfriendId", auth, upload.single("photo"), async (req, res) => {
  try {
    const { girlfriendId } = req.params;
    const { name, details } = req.body;
    ...
    const user = await User.findById(req.user.id);
    const girlfriend = user.girlfriends.id(girlfriendId);
    if (!girlfriend) return res.status(404).json({ message: "Proposal not found" });
    ...
    if (req.file) {
      girlfriend.photo = req.file.path;
    }

    await user.save();
    res.json({ success: true, message: "Updated!", girlfriend });
  } catch (err) {
    res.status(500).json({ message: "Server error" });
  }
});
